...
function showSpellTargetModal(spellName, spellLevel) {
    ...
    const targets = State.characters.filter(c => c.id !== char.id && (c.campaignId || 'default') === campaignId);
        ...
        <div class="modal-overlay" onclick="closeModal(event)">
            <div class="modal-content" onclick="event.stopPropagation()">
                <div class="p-6">
                    ...
                    <p class="text-sm text-amber-700 mb-3">Select a character from your campaign:</p>
                    <div class="space-y-2 max-h-64 overflow-y-auto">
                        ${targets.length === 0 ? '<p class="text-amber-700 italic">No other characters in this campaign.</p>' : ''}
                        ${targets.map(t => `
                            <div class="flex justify-between items-center p-3 bg-amber-100 rounded border border-amber-300 cursor-pointer hover:bg-amber-200" onclick="castSpellOnTarget('${spellName}', ${spellLevel}, '${t.id}')">
                                <div>
                                    <div class="font-bold text-amber-900">${t.name}</div>
