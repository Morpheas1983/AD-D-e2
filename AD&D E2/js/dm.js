...
function renderDMView(container) {
    ...
    const campaignChars = hasCampaignFilter ? allChars.filter(c => c.campaignId === State.campaignId) : allChars;
                ...
                <div class="parchment scroll-border p-4 mb-4 fade-in">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <button onclick="changeView('campaigns')" class="btn-fantasy px-3 py-2 rounded text-sm">
                                <i class="fas fa-scroll mr-1"></i>Campaigns
                            </button>
                            <button onclick="changeView('bestiary')" class="btn-fantasy px-3 py-2 rounded text-sm">
                                <i class="fas fa-paw mr-1"></i>Bestiary
                            </button>
                            <button onclick="changeView('monsters')" class="btn-fantasy px-3 py-2 rounded text-sm">
                                <i class="fas fa-dragon mr-1"></i>Monsters
                            </button>
                            <button onclick="changeView('tactical')" class="btn-fantasy px-3 py-2 rounded text-sm">
                                <i class="fas fa-chess-board mr-1"></i>Tactical Map
                            </button>
                            <button onclick="changeView('users')" class="btn-fantasy px-3 py-2 rounded text-sm">
                                <i class="fas fa-users-cog mr-1"></i>Users
                            </button>
                            <button onclick="showAssignCharModal()" class="btn-fantasy px-3 py-2 rounded text-sm">
                    ...
                    <div class="parchment scroll-border p-4 fade-in cursor-pointer" onclick="dmViewCharacter('${char.id}')">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                ...
                                <p class="text-xs text-amber-700">${char.player || 'Unassigned'} &middot; ${char.race} ${char.class}</p>
                            </div>
                        ...
                        ${unassigned.map(char => `
                            <div class="flex items-center justify-between p-3 bg-amber-100 rounded border border-amber-300">
                                <div>
                                    ...
                                    <div class="text-xs text-amber-700">${char.race} ${char.class} &middot; Level ${char.level}</div>
